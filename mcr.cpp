...
+ no input validation 
+ memory corruption when i = 3 or j=3 is inputted when game[i]j] is assigned with a value. 
+ missing tie message sometimes when tie case is i<>3.

Code Smells:
+ Text for Player 1 and Player 2 are duplicated.
+ variables player1 and player2 should be used when assigning values to game[i][j].


When needing to be compiled online in C++ 14. use the following version:
